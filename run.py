...
def assign_dice_to_task(dice_pool, task):
    ...
    while not task.complete:
        ...
        if index == "pass":
            print("Not assigning any dice from this roll.")
            print("Removing one die form your dice pool.")
            dice_pool.pop()
            return dice_pool, task
        die = dice_pool[index-1]
        print(f"Assigning die {index} with {str(die)} to {task.remaining}.")
        try:
            #should this validation be done elsewhere?
            task.assign_die(die)
